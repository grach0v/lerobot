"""A2 Camera configurations.

This module contains camera intrinsics and poses for various RealSense cameras
used in the A2 simulation environment.
"""
...
class RealSenseL515:
    ...
    # Primary views: front, left, right (used for point cloud generation)
    front_position = (1.0, 0, 0.5)
    front_rotation = (np.pi / 4, np.pi, -np.pi / 2)
    front_rotation = p.getQuaternionFromEuler(front_rotation)
    left_position = (0.2, 0.35, 0.5)
    left_rotation = (np.pi / 4.5, np.pi, np.pi / 4)
    left_rotation = p.getQuaternionFromEuler(left_rotation)
    right_position = (0.2, -0.35, 0.5)
    right_rotation = (np.pi / 4.5, np.pi, 3 * np.pi / 4)
    right_rotation = p.getQuaternionFromEuler(right_rotation)

    # Alternative top-down views (used for VLA training)
    top_position = (0.5, 0, 0.6)
    top_rotation = (0, np.pi, -np.pi / 2)
    top_rotation = p.getQuaternionFromEuler(top_rotation)
    side_left_position = (0.2, 0.35, 0.5)
    side_left_rotation = (np.pi / 4.5, np.pi, np.pi / 4)
    side_left_rotation = p.getQuaternionFromEuler(side_left_rotation)
    side_right_position = (0.8, -0.35, 0.5)
    side_right_rotation = (np.pi / 4.5, np.pi, -3 * np.pi / 4)
    side_right_rotation = p.getQuaternionFromEuler(side_right_rotation)
    ...
    CONFIG = [
        ...
        },
        {
            "image_size": image_size,
            "intrinsics": intrinsics,
            "position": side_left_position,
            "rotation": side_left_rotation,
            "zrange": (0.01, 9.0),
            "noise": False,
        },
        {
            "image_size": image_size,
            "intrinsics": intrinsics,
            "position": side_right_position,
            "rotation": side_right_rotation,
            "zrange": (0.01, 9.0),
            "noise": False,
        },
        {
...
class RealSenseD435:
    ...
    front_rotation = (np.pi / 4, np.pi, -np.pi / 2)
    front_rotation = p.getQuaternionFromEuler(front_rotation)
    left_position = (0.2, 0.3, 0.5)
    left_rotation = (np.pi / 4.5, np.pi, np.pi / 4)
    left_rotation = p.getQuaternionFromEuler(left_rotation)
    right_position = (0.2, -0.3, 0.5)
    right_rotation = (np.pi / 4.5, np.pi, 3 * np.pi / 4)
    right_rotation = p.getQuaternionFromEuler(right_rotation)
