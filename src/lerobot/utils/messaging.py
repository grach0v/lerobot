...
class TorchSerialize:
    def encodes(self, o: torch.Tensor | np.ndarray) -> dict:
        if isinstance(o, torch.Tensor):
            np_data = o.detach().cpu().numpy()
            return {
                "data": np_data.tobytes(),
                "dtype": np_data.dtype.str,
                "encoding": "raw_bytes",
                "shape": o.shape,
                "type": "tensor",
            }
    ...
    def decodes(self, o: dict) -> torch.Tensor | np.ndarray:
        dtype = o["dtype"]
        t = o["type"]
...
class JsonLikeSerialize:
    def encodes(self, o: torch.Tensor | np.ndarray | Any) -> dict:
        if not isinstance(o, list):
            data = o.tolist()
            return {"data": data, "dtype": "tensor", "encoding": "json"}
...
def encode_image_data(obj: torch.Tensor | np.ndarray) -> dict:
    return {"__image_data__": True, "as_str": img_serializer.encodes(obj)}


def encode_image_data_json(obj: torch.Tensor | np.ndarray) -> list:
    return img_serializer.encodes(obj)


def decode_image_data(obj: dict) -> torch.Tensor | np.ndarray:
...
def unpack_msg(packed: bytes, with_header: bool = False) -> Any:
    ...
    Args:
        packed (bytearray): bytearray containing a msgpack message
    """
