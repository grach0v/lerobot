#!/usr/bin/env python
"""Debug script for pickplace grasp execution with better grasp pose."""
...
# Try grasp with top-down orientation (quaternion [1, 0, 0, 0] is identity/straight down)
# The strawberry is at z=0.02, so grasp at z=0.025 (just above surface)
grasp_pose = np.array([pos[0], pos[1], 0.025, 1.0, 0.0, 0.0, 0.0])
print(f"\nAttempt 1 - Top-down grasp at: {grasp_pose[:3]}")
...
if grasp_success and grasped_id in env.target_obj_ids:
    print("SUCCESS: Grasped target object!")
else:
    print(f"FAIL: Did not grasp target")

    # Try different grasp positions
    for z_offset in [0.02, 0.03, 0.015]:
        grasp_pose = np.array([pos[0], pos[1], z_offset, 1.0, 0.0, 0.0, 0.0])
        print(f"\nAttempt z={z_offset} at: {grasp_pose[:3]}")

        grasp_success, grasped_id, min_dist = env.grasp(grasp_pose, follow_place=True)
        print(f"Grasp result: success={grasp_success}, grasped_id={grasped_id}")
